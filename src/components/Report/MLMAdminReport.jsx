                          ...
                          {item.new_members_details && item.new_members_details.length > 0 ? (
                            item.new_members_details.map((member, idx) => (
                              <div key={idx} className="text-sm mb-1">
                                <span className="font-medium">{member.member_id}</span>: {member.full_name || member.username}
                              </div>
                            ))
                          ) : (
                            'N/A'
      ...
      case 'sales':
        return (
          <div>
                      ...
                      <td className="py-2 px-4 border-b">
                        <div className="max-h-24 overflow-y-auto">
                          {item.product_details && item.product_details.length > 0 ? (
                            item.product_details.map((product, idx) => (
                              <div key={idx} className="text-sm mb-1">
                                <span className="font-medium">{product.name}</span>
                                {product.quantity && <span className="ml-1">({product.quantity}x)</span>}
                              </div>
                            ))
                          ) : (
                            'N/A'
            ...
            {/* Product list section for sales report */}
            {reportData.product_list && reportData.product_list.length > 0 && (
              <div className="mt-6">
                <h3 className="text-lg font-semibold mb-3">Top Products</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {reportData.product_list.map((product, idx) => (
                    <div key={idx} className="bg-white border rounded-lg p-4">
                      <p className="font-medium">{product.name}</p>
                      <div className="flex justify-between mt-2">
                        <span className="text-sm text-gray-600">Quantity: {product.total_quantity}</span>
                        <span className="text-sm text-gray-600">Revenue: ₹{product.total_sales?.toFixed(2) || '0.00'}</span>
                      </div>
                    </div>
                  ))}
                </div>
