...
const PaymentHandler = ({ amount }) => {
  ...
  const createOrder = async (isCOD = false) => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/orders/create/`, {
      ...
      body: JSON.stringify({
        ...
        total_amount: amount,
        total_bp: totalBPPoints,
        orderType: isCOD ? 'COD' : 'ONLINE', // Set order type based on payment method
      })
    });
  ...
  const createCODOrder = async () => {
    try {
      ...
      // Create order with COD flag
      const orderData = await createOrder(true);
      setOrderCreated(true);
      ...
      router.push(`/thankyou?order_id=${orderData.order_id}`);
    } catch (error) {
  ...
  const handleOnlinePayment = async () => {
    ...
    try {
      ...
      // Create order first
      const orderData = await createOrder(false); // false = online payment
      setOrderCreated(true);
  ...
  // Handle main payment action based on selected method
  const handlePayment = () => {
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      {/* COD notice/disclaimer */}
      {paymentMethod === 'cod' && (
        <div className="text-sm bg-yellow-50 border border-yellow-100 p-3 rounded-md">
          <p className="font-semibold">Cash on Delivery Terms:</p>
          <p>Pay with cash at the time of delivery. A small COD fee may apply in some areas.</p>
        </div>
      )}
      ...
      <button
        onClick={handlePayment}
        disabled={isDisabled}
        className="w-full bg-[#6B8E5F] text-white py-3 rounded-lg hover:bg-[#5c7a51] 
                 transition-colors disabled:opacity-50 disabled:cursor-not-allowed
                 flex items-center justify-center gap-2"
      >
        {loading ? (
